...
BOARD_ID = 'switch_board_ipc_db_validate'
BOARD_NAME = 'Switch Board IPC DB Validate'
LOG = ROOT / '.tmp_validate_db_import_chain.log'
...
subprocess.run([
    ...
    'public/examples/switch_board_ipc.json', BOARD_ID, BOARD_NAME,
], cwd=ROOT, check=True)
...
try:
    ...
    assert BOARD_ID in ids, 'imported db board missing from list'
    ...
    assert target, 'no component with netIds found'
    ...
    assert total_top >= 200, ('top geometry too low', total_top)
    ...
    assert total_bottom >= 20, ('bottom geometry too low', total_bottom)

    rel = fetch_json(f'/api/boards/{BOARD_ID}/relations/component/{urllib.request.quote(target["id"])}')
    assert len(rel['nets']) >= 1, 'db relations nets empty'
    assert len(rel['traces']) >= 1, 'db relations traces empty'

    page = fetch_text(f'/board/{BOARD_ID}')
    assert BOARD_NAME in page

    print('validate_db_import_chain: OK')
finally:
    proc.terminate()
    try:
