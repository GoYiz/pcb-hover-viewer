...
- conclusions:
  ...
  - `externalBucketProjection` now exactly matches the C++ baseline for all main buckets on the LED sample: `board_outline / copper / via / zone / graphics`.
...
- ours external bucket projection: board_outline=213, copper=120, via=12, zone=71, graphics=559
...
- conclusions:
  ...
  - The remaining `graphics` gap (`559` vs `729`) is now isolated as a representation-gap issue, not a missing semantic bucket.
  - The C++ tool exports many board-level `Dwgs.User` primitives individually, while the viewer-oriented importer preserves more geometry as higher-level paths and viewer-facing arrays.
...
- Remaining work on `graphics/drill` is optional refinement, not missing structure: future improvements would tighten primitive-level parity for larger boards like `switch`, but the current long-term viewer model and import pipeline are already stable and explicit.
